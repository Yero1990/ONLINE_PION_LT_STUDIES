...
void setTimeWindows(int run, TString daq_mode="coin", Bool_t set_refTimes=true, Bool_t debug=false)
{
  ...
  //Opern Ref Time Windows (used to set ref. times)
  ...
  if(set_refTimes) {
    ...
    outROOT = new TFile(Form("./Time_cuts_test%d/refTime_%s_histos_run%d.root", run, rtype.Data(), run), "recreate"); 
  }
